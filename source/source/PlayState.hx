...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Exterminated', 0.09], //From 0% to 8%
		['The Strongest Rap', 0.1], //Baka. 9%
		['Exterminated', 0.2], //From 10% to 2%
		['Sealed', 0.4], //From 20% to 39%
		['Pichuun', 0.5], //From 40% to 49%
		['Baka', 0.6], //From 50% to 59%
		['Grazed', 0.69], //From 60% to 68%
		['Lol', 0.7], //69%
		['Magical!!', 0.8], //From 70% to 79%
		['Phantasmal!!', 0.9], //From 80% to 89%
		['LUNATIC', 0.99], //From 90% to 98%
		['True Baka, Baka!', 1], //baka. 99%
		['feels like a 1CC', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public static var daPixelZoom:Float = 9;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				cirnoBop = new BGSprite('Cirnobop', -550, 580, 1.5, 1.5, ['Cirno Bop0']); //god i hope this works, this is my first time trying to code in a stage element that was never there originally
				cirnoBop.setGraphicSize(Std.int(cirnoBop.width * 2.2));
				cirnoBop.updateHitbox(); //idk what this does im just hoping it does what its supposed to do which is resize athing please work oh dear lord
				add(cirnoBop);																//oh cool it works

			case 'spooky': //Week 2
				if(!ClientPrefs.lowQuality) {
					halloweenBG = new BGSprite('halloween_bg', -650, -200, ['halloweem bg0', 'halloweem bg lightning strike']);
				} else {
					halloweenBG = new BGSprite('halloween_bg_low', -650, -200);
				}
				halloweenBG.setGraphicSize(Std.int(halloweenBG.width * 1.2));
			...
			case 'philly': //Week 3
				if(!ClientPrefs.lowQuality) {
					var bg:BGSprite = new BGSprite('philly/sky', -300, -175, 0, 0);
					add(bg);
				}

				var city:BGSprite = new BGSprite('philly/city', -260, 0, 0.3, 0.3);
				...
				for (i in 0...5)
				{
					var light:BGSprite = new BGSprite('philly/win' + i, -260, 0, 0.3, 0.3);
					light.visible = false;
				...
				if(!ClientPrefs.lowQuality) {
					var streetBehind:BGSprite = new BGSprite('philly/behindTrain', 10, 125);
					add(streetBehind);
				}

				phillyTrain = new BGSprite('philly/train', 3000, 360);
				add(phillyTrain);
				...
				var street:BGSprite = new BGSprite('philly/street', -240, 125);
				add(street);

			case 'limo': //Week 4
				var skyBG:BGSprite = new BGSprite('limo/limoSunset', -300, -50, 0.15, 0.15);
				add(skyBG);
				...
				if(!ClientPrefs.lowQuality) {
					...
					bgLimo = new BGSprite('limo/bgLimo', -150, 510, 0.4, 0.4, ['background limo pink'], true);
					add(bgLimo);
				...
				fastCar = new BGSprite('limo/fastCarLol', -150, 160);
				fastCar.active = true;
				limoKillingState = 0;
			...
			case 'mall': //Week 5 - Cocoa, Eggnog
				...
				var fgSnow:BGSprite = new BGSprite('christmas/fgSnow', -700, 700);
				add(fgSnow);
			...
			case 'mallEvil': //Week 5 - Winter Horrorland
				...
				var evilSnow:BGSprite = new BGSprite('christmas/evilSnow', -500, 600);
				add(evilSnow);
			...
			case 'school': //Week 6 - Senpai, Roses
				...
				var bgSchool:BGSprite = new BGSprite('weeb/weebSchool', repositionShit, 50, 0.6, 0.90);
				add(bgSchool);
				bgSchool.antialiasing = false;

				var bgStreet:BGSprite = new BGSprite('weeb/weebStreet', repositionShit, 10, 0.95, 0.95);
				add(bgStreet);
				...
				bgTrees.scrollFactor.set(0.55, 0.85);
				bgTrees.setGraphicSize(Std.int(bgTrees.width * 2));
				...
				if(!ClientPrefs.lowQuality) {
					bgGirls = new BackgroundGirls(-100, 200);
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "Too lazy To Play Lol", 32);
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'transient' | 'native-faith' | 'ancient-battlefield':
					if(daSong == 'native-faith') FlxG.sound.play(Paths.sound('ANGRY'));
					schoolIntro(doof);
		...
		DiscordClient.changePresence(detailsText, "Rap battling on the song: " + SONG.song + " [" + storyDifficultyText + "]");
		#end
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		var songName:String = Paths.formatToSongPath(SONG.song);
		if (songName == 'transient' || songName == 'native-faith' || songName == 'ancient-battlefield')
		{
			remove(black);

			if (songName == 'ancient-battlefield')
			{
				camHUD.visible = false;
				add(red);
			}
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					if (Paths.formatToSongPath(SONG.song) == 'ancient-battlefield')
					{
						add(senpaiEvil);
						senpaiEvil.alpha = 0;
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				switch (swagCounter)
				{
					...
					case 1:
						var ready:FlxSprite = new FlxSprite().loadGraphic(Paths.image(introAlts[0],introDir));
					...
					case 2:
						var set:FlxSprite = new FlxSprite().loadGraphic(Paths.image(introAlts[1],introDir));
					...
					case 3:
						var go:FlxSprite = new FlxSprite().loadGraphic(Paths.image(introAlts[2],introDir));
						go.scrollFactor.set();
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, "Rap battling on the song: " + SONG.song + " [" + storyDifficultyText + "]", true, songLength);
		#end
...
songSpeed = SONG.speed;
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, "Rap battling on the song: " + SONG.song + " [" + storyDifficultyText + "]", true, songLength - Conductor.songPosition - ClientPrefs.noteOffset);
			}
			else
			{
				DiscordClient.changePresence(detailsText, "Rap battling on the song: " + SONG.song + " [" + storyDifficultyText + "]");
	...
	override public function onFocusLost():Void
	{
		...
		if (health > 0 && !paused)
		{
			DiscordClient.changePresence(detailsPausedText, "Rap battling on the song: " + SONG.song + " [" + storyDifficultyText + "]");
		}
		#end
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.SEVEN && !endingSong && !inCutscene)
		{
			...
			#if desktop
			DiscordClient.changePresence("Forever working on the mod", null, null, true);
			#end
		}
	...
	function doDeathCheck() {
		if (health <= 0 && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Game Over - " + detailsText, "Lost on:" + SONG.song + " (" + storyDifficultyText + ")");
				#end
				isDead = true;
				return true;
			}
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['Nitori_nomiss', 'Door_nomiss', 'Witchcraft_nomiss', 'Sakyua_nomiss',
				'GensokyoParty_nomiss', 'MoriaShrine_nomiss', 'ur_bad',
				'ur_good', 'hype', 'two_keys', 'toastie', 'botplayer', 'baka']);
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					var winterHorrorlandNext = (Paths.formatToSongPath(SONG.song) == "hedonism");
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.loadGraphic(Paths.image(pixelShitPart1 + daRating + pixelShitPart2,pixelDir));
		rating.screenCenter();
		...
		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image(pixelShitPart1 + 'combo' + pixelShitPart2,pixelDir));
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(pixelShitPart1 + 'num' + Std.int(i) + pixelShitPart2,pixelDir));
	...
	private function checkForAchievement(achievesToCheck:Array<String>):String {
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName)) {
				...
				{
					case 'Nitori_nomiss' | 'Door_nomiss' | 'Witchcraft_nomiss' | 'Sakyua_nomiss' | 'GensokyoParty_nomiss' | 'MoriaShrine_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'Nitori':
									if(achievementName == 'Nitori_nomiss') unlock = true;
								case 'Door':
									if(achievementName == 'Door_nomiss') unlock = true;
								case 'Witchcraft':
									if(achievementName == 'Witchcraft_nomiss') unlock = true;
								case 'Sakyua':
									if(achievementName == 'Sakyua_nomiss') unlock = true;
								case 'Shrine':
									if(achievementName == 'GensokyoParty_nomiss') unlock = true;
								case 'Pixel-Shrine':
									if(achievementName == 'MoriaShrine_nomiss') unlock = true;
							}
						}
					case 'ur_bad':
					...
					case 'baka':
						if(ratingPercent > 0.08 && ratingPercent < 0.1 && !practiceMode && !cpuControlled) {
							unlock = true;
						}	
				}
